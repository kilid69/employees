...
CREATE TABLE dept_managers_gender_since_1990 (manager INT, gender ENUM('m','f'), dept_name VARCHAR(50), years INT);
...
the_loop: LOOP
		...
		SELECT m.emp_no AS manager, e.gender AS manager_gender, d.dept_name AS dept_name, a
	FROM t_dept_manager m LEFT JOIN t_departments d ON m.dept_no = d.dept_no LEFT JOIN t_employees e ON e.emp_no = m.emp_no
...
WITH CTE AS (
SELECT e.gender gender, s.salary salary, s.from_date, s.to_date, d.dept_name dept_name, y.calender_year calender_year
FROM t_salaries s 
...
SELECT gender, AVG(salary), dept_name, YEAR(calender_year) FROM CTE
...
SELECT e.gender, d.dept_name, AVG(s.salary) salary, YEAR(s.from_date) calender_year
from t_salaries s join t_employees e ON e.emp_no = s.emp_no
...
BEGIN
	SELECT e.gender, d.dept_name, ROUND(AVG(s.salary), 2) salary
	from t_salaries s join t_employees e ON e.emp_no = s.emp_no
	join t_dept_emp de ON de.emp_no = e.emp_no
	join t_departments d ON d.dept_no = de.dept_no
